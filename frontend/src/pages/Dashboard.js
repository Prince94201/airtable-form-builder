...
function Dashboard() {
  ...
  const fetchForms = async () => {
    try {
      const response = await axios.get('http://localhost:4000/api/forms', { withCredentials: true });
  ...
  const fetchRecords = async (baseId, tableId) => {
    setDataLoading(true);
    try {
      const response = await axios.get(`http://localhost:4000/api/bases/${baseId}/${tableId}/records`, { withCredentials: true });
